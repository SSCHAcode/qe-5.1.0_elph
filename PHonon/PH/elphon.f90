...
SUBROUTINE elphsum_simple
  ...
  file_elphmat=TRIM(fildyn)//'.elph.d.mat.'//TRIM(int_to_char(current_iq))

  open(unit=27,file=file_elphmat)
...
100 CONTINUE
  ...
  DO isig = 1, el_ph_nsigma
     ...
     !
     ! Print degauss and el_ph_sum
     !

     write(unit=27,fmt=*) degauss1
     write(unit=27,fmt=*) dosef

     do nu=1,3*nat
        do mu=1,3*nat
           write(unit=27,fmt=*) el_ph_sum(nu,mu)
        enddo
     enddo
...
9005 FORMAT(5x,'DOS =',f10.6,' states/spin/Ry/Unit Cell at Ef=', &
          &       f10.6,' eV')
9006 FORMAT(5x,'double delta at Ef =',f20.6)
9010 FORMAT(5x,'lambda(',i5,')=',f12.4,'   gamma=',f12.2,' GHz')
  !
  !
